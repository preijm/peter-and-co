...
html = html.replace(
  `    const src = document.getElementById('mondriaan-src').textContent;\n    const compiled = Babel.transform(src, { presets: ['react'] }).code;`,
  `    const compiled = document.getElementById('mondriaan-src').textContent;`
);
...
// 2. Pre-compile Mondriaan lazy-load blob (stays as text/plain, but no JSX)
html = html.replace(
  /(<script type="text\/plain" id="mondriaan-src">)([\s\S]*?)(<\/script>)/,
  (_, open, src, close) => {
    console.log('Compiling Mondriaan blob…');
    const { code } = babel.transformSync(src.trim(), babelConfig);
    return `${open}\n${code}\n${close}`;
  }
);
